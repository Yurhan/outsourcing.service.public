...
export function putContactRouteHandler(req: IAppRequest, res: IJsonResponse): void {
  let logger = req.kernel.get<ILoggerFactory>(TYPES.LOGGER_FACTORY).getLogger('actions.timer.putContactRouteHandler');
  logger.info('Put /Contact');
  ...
  res.jsonPromise(
    unitOfWork.beginAutoCommitTransaction(() => service.add(contact))
  );
}
