...
export function putCompanyServicesRouteHandler(req: IAppRequest, res: IJsonResponse): void {
  let logger = req.kernel.get<ILoggerFactory>(TYPES.LOGGER_FACTORY).getLogger('actions.timer.putCompanyServicesRouteHandler');
  logger.info('Put /CompanyServices');
